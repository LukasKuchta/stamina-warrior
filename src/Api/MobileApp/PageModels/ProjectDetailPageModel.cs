...
public partial class ProjectDetailPageModel : ObservableObject, IQueryAttributable, IProjectTaskPageModel
{
    ...
    {        
        if (query.TryGetValue("id", out var val))
        {
            int id = Convert.ToInt32(val, CultureInfo.InvariantCulture);
            _ = LoadData(id).FireAndForgetSafeAsync(_errorHandler);
        }
        else if (query.ContainsKey("refresh"))
        {
            _ = RefreshData().FireAndForgetSafeAsync(_errorHandler);
        }
        else
        {
            _ = Task.WhenAll(LoadCategories(), LoadTags()).FireAndForgetSafeAsync(_errorHandler);
            _project = new()
            {
                Tags = [],
                Tasks = []
