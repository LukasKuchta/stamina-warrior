...
public partial class TaskDetailPageModel : ObservableObject, IQueryAttributable
{
    ...
    {
        _ = LoadTaskAsync(query).FireAndForgetSafeAsync(_errorHandler);
    }
    ...
    private async Task LoadTaskAsync(IDictionary<string, object> query)
    {
        ...
        {
            taskId = Convert.ToInt32(val, CultureInfo.InvariantCulture);
    ...
    private async Task Save()
    {
        ...
        {
            _ = _taskRepository.SaveItemAsync(_task).FireAndForgetSafeAsync(_errorHandler);
        }
